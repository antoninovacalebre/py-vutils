...
def formatter(value):
    ...
    '''
    myclassifier = classifier(value)
    if myclassifier == None:
        # Don't know how to classify this value
        return None

    scaled = value / myclassifier['scalar']
    return (scaled, myclassifier['short_suffix'], myclassifier['long_suffix'], myclassifier['scalar'])

def format(value, precision=4, long_form=False, separator=''):
    '''
    ...
    '''
    scaled, short_suffix, long_suffix, scalar = formatter(value)
